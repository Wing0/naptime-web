...
- A Worker named `naptime-experiments` handles retired Paid routes, first-party events, and legacy `/free.html*` routes. The supported product page is `/`.
- Retired Paid paths redirect to `/`. Free experiment routing remains disabled with `ENABLE_FREE_EXPERIMENT = false` so the worldwide Free launch uses the stable homepage.
