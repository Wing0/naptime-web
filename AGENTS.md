...
- Paid experiment routing is enabled with `ENABLE_PAID_EXPERIMENT = true`; Free experiment routing remains disabled with `ENABLE_FREE_EXPERIMENT = false` so the worldwide Free launch uses the stable homepage. Test paid forced variants after every Worker deploy.
