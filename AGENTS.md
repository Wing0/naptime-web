...
- A Worker named `naptime-experiments` handles first-party events, legacy `/free.html*` routes, and live redirects from retired Paid paths to `/`. Verify live routing after any Worker change. The supported product page is `/`.
- Free experiment routing remains disabled with `ENABLE_FREE_EXPERIMENT = false` so the worldwide Free launch uses the stable homepage.
